...
export async function POST(req) {
  ...
  try {
    ...
    return NextResponse.json({ error: "Failed to upload image. Check R2 credentials and endpoint." }, { status: 500 });
  }

  const publicBase =
    process.env.R2_PUBLIC_URL?.replace(/\/$/, "") ||
    `${process.env.R2_ENDPOINT.replace(/\/$/, "")}/${process.env.R2_BUCKET_NAME}`;
